...
{
    public class ElementEllipse : IVectorElement
    {
        ...
        public void                 draw(Graphics aGraphics)
        {
            ...
            if (mFillColor.A > 0)
            {
                aGraphics.FillEllipse(mFillBrush, mX, mY, mWidth, mHeight);
            }

            if (mBorderWidth != 0)
            {
                aGraphics.DrawEllipse(mBorderPen, mX, mY, mWidth, mHeight);
            }
        }
        ...
        public void                 drawSelected(Graphics aGraphics)
        {
            ...
            if (mBorderWidth <= 1)
            {
                aGraphics.DrawEllipse(VectorImagePanel.mSelectionPen, lRect);
            }
            else
            {
                int lDelta = (int)mBorderWidth / 2;
                aGraphics.DrawEllipse(VectorImagePanel.mSelectionPen, lRect.X + lDelta, lRect.Y + lDelta, lRect.Width - mBorderWidth, lRect.Height - mBorderWidth);
                aGraphics.DrawEllipse(VectorImagePanel.mSelectionPen, lRect.X - lDelta, lRect.Y - lDelta, lRect.Width + mBorderWidth, lRect.Height + mBorderWidth);
            }
        ...
        public bool                 testHit(Point aPoint)
        {
            ...
            if (mFillColor.ToArgb() == Color.Transparent.ToArgb() && mWidth > lDoubleDelta && mHeight > lDoubleDelta)
            {
                ...
                {
                    lPath.AddEllipse(mX + lHitDelta, mY + lHitDelta, mWidth - lDoubleDelta, mHeight - lDoubleDelta);
                    if (lPath.IsVisible(aPoint))
                    {
                        return false;
                    }
                }
