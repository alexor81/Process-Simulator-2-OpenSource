...
namespace SimulationObject.Pipeline.Pump.Panels.PumpState
{
	public partial class PumpStatePanel : UserControl, IPanel
	{
		...
		private void            redraw()
		{
			...
			if (mPump.Remote)
			{
				mGraphics.DrawImage(Properties.Resources.Pmp_DC, mImageRect);
			}
			else
			{
				mGraphics.DrawImage(Properties.Resources.Pmp_NDC, mImageRect);
			}

			if (mPump.On)
			{
				mGraphics.DrawImage(Properties.Resources.Pmp_On, mImageRect);
			}
			else
			{
				mGraphics.DrawImage(Properties.Resources.Pmp_Off, mImageRect);
			}

			if (mPump.Power == false || mPump.Alarm || mPump.mEsdCMD || mPump.IgnoreCommands)
			{
				mGraphics.DrawImage(Properties.Resources.Alarm, mImageRect);
			}
